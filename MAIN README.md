...
|#|Project                                                                             |Description                                                                                                                                                                       |
|-|------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|1|[SIEM Log Analysis & Threat Detection](./SIEM-Log-Analysis-Splunk/)                 |Investigated 3 real-world SOC alerts in Splunk — brute force, C2 persistence, and SSH compromise — using 70+ SPL queries across Linux secure logs, Sysmon, and Windows Event Logs |
|2|[Network Traffic Analysis & Security Log Investigation](./Network-Traffic-Analysis/)|Detected FTP credential interception, unauthorized Windows account creation, and a full ARP spoofing MITM attack chain using Wireshark, Tcpdump, and PowerShell Event Log analysis|
|3|[Snort IDS Rule Development](./Snort-IDS/)                                          |Developed custom Snort detection rules for NIDS/NIPS environments targeting specific attack signatures and protocol anomalies                                                     |

### 🔴 Malware Analysis & DFIR

|#|Project                                                                        |Description                                                                                                                                                                                                                                |
|-|-------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|4|[Malware Analysis — C2 Implant & Ransomware](./Malware-Analysis/)              |Full static and dynamic analysis of two malware samples — a C2 spy implant (WS2_32 + IPHLPAPI imports, beacon to 192.229.231.5) and PhotoCrypter ransomware (Base64-encoded ransom note, Pastebin C2, HKCU Run persistence)                |
|5|[Digital Forensics & IR — Memory Analysis + Narcos Case](./DFIR-Investigation/)|Volatility memory forensics across two dumps (trojan process injection, credential dumping via winpmem) and a multi-device criminal DFIR investigation covering Discord artifacts, steganographic images, TrueCrypt volumes, and Quasar RAT|

### 🟠 Offensive Security

|#|Project                                                                         |Description                                                                                                                                                                                                                                                           |
|-|--------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|6|[Penetration Testing — Web + Network](./Penetration-Testing/)                   |Web application assessment confirming SQLi, stored/reflected XSS, broken access control, and IDOR across OWASP Juice Shop and WebGoat; network boot-to-root achieving SYSTEM via EternalBlue (MS17-010) with post-exploitation credential dumping and lateral movement|
|7|[Python Security Automation & Offensive Scripting](./Python-Security-Scripting/)|Built a dual-mode ARP reconnaissance tool (passive sniffing + active scanning via Scapy) and a four-module offensive framework — multi-threaded port scanner, SSH brute force (Paramiko), HTTP directory buster, and encrypted reverse shell (Fernet)                 |

### 🟣 GRC & Governance

|#|Project                                            |Description                                                                                                                                                                                                          |
|-|---------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|8|[GRC Implementation Project](./GRC-Implementation/)|Designed and implemented an enterprise cybersecurity GRC program across ISO 27001, NIST CSF, COBIT, and CIS Controls — covering risk assessment, control mapping, and compliance alignment to GDPR, PCI DSS, and DORA|

### 🟡 Academic Projects

|# |Project                                                               |Description                                                                                                                                                                                   |
|--|----------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|9 |[Online Banking Portal — Secure Transaction System](./Banking-Portal/)|Built a web banking application with dual-password architecture and grid-based debit card verification — separating login authentication from transaction authorization                       |
|10|[YouTube Spam Detection — ML Classification](./Spam-Detection/)       |Trained and compared five supervised learning models (Naive Bayes, SVM, Logistic Regression, Neural Network, Ensemble) for YouTube comment spam classification using TF-IDF feature extraction|
|11|[Smoke & Gas Detection System — IoT](./Gas-Detection-IoT/)            |Built a real-time gas monitoring system using MQ-2 sensor and NodeMCU ESP8266 with threshold-based alerts and ThingSpeak cloud data logging                                                   |
